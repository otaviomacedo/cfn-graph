...
describe('Integration Tests', () => {
  ...
  describe('Node Movement Scenarios', () => {
    ...
    test('should preserve Ref references when moving both resources to different stack', () => {
      const stack1: CloudFormationTemplate = {
        Resources: {
          Bucket: {
            Type: 'AWS::S3::Bucket',
    ...
    test('should preserve Fn::GetAtt references when moving both resources to different stack', () => {
      const stack1: CloudFormationTemplate = {
        Resources: {
          Bucket: {
            Type: 'AWS::S3::Bucket',
          ...
          Function: {
            ...
            Properties: {
              Environment: {
                Variables: {
                  BUCKET: {'Fn::GetAtt': ['Bucket', 'BucketName']}
                }
              }
            },
            DependsOn: 'Bucket'
      ...
      // Verify Ref remains unchanged (not converted to ImportValue)
      expect(stack2Template.Resources.Function.Properties!.Environment.Variables.BUCKET).toEqual({'Fn::GetAtt': ['Bucket', 'BucketName']});
